...
export async function POST(req) {
  try {
    ...
    const artistEntity = {
      username: toKebabCase(data.stageName),
      name: data.stageName,
      stageName: data.stageName,
      type: data.type ? data.type : 'artist',
      agreed: false,
      verified: false,
    ...
    const result = await collection.insertOne(artistEntity);
    console.log(
    ...
    return NextResponse.json(
      {
        message: 'Inserted artist successfully',
        data: assoc('stageName', artistEntity.stageName, result),
      },
      { status: SUCCESSFUL }
    );
  } catch (e) {
