...
var Sidebar = function () {
    ...
    var _a = useTheme(), replaceClassName = _a.replaceClassName, sidebarSkin = _a.sidebarSkin;
    /**
     *
    ...
    var isActiveRoute = function (href) {
        var isActivePage = pathname.split('/')[2] === href.split('/')[2];
    ...
    var navLink = function (nav) {
        return (nav.title ? (<>
                ...
                </>) : (<Link href={nav.href} className={classNames('nav-link d-flex align-items-center', isActiveRoute(nav.href) && 'active')}>
                    <nav.icon size={20}/>
                    <span className={replaceClassName('ms-3')}>{nav.name}</span>
                </Link>));
    };
    return (<aside id='sidebar' data-sidebar={sidebarSkin}>
            ...
            <Scrollbar className='sidebar-body'>
                <nav className='navbar d-block p-0'>
                    <ul className='navbar-nav'>
                        {NAVBAR.map(function (nav, index) { return (<li key={index} className={classNames('nav-item', nav.title && 'nav-item--head')}>
                                {navLink(nav)}
                            </li>); })}
                    </ul>
                </nav>
