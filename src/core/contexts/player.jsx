...
var Player = function (_a) {
    ...
    var _b = useLocalStorage(SONG_KEY, []), songs = _b[0], setSongs = _b[1];
    var enqueueSnackbar = useSnackbar().enqueueSnackbar;
    var status = useScript('/js/amplitude.min.js', {
        removeOnUnmount: false,
        id: 'Amplitude',
    });
    var _c = useState({}), activeSong = _c[0], setActiveSong = _c[1];
    ...
    var getCurrentSong = function () { return Amplitude.getActiveSongMetadata(); };
    // Retrieve the index of a song based on its ID
    var getSongIndex = function (id) {
        return SONGS.findIndex(function (song) { return song.id === id; });
    };
    // Retrieve song data compatible with the player.
    var getSongObject = function (song) {
        return __assign(__assign({}, song), { name: song.title, artist: getPersonName(song === null || song === void 0 ? void 0 : song.artists) || '', url: song.src, cover_art_url: song.cover });
    };
    ...
    var initPlayer = function (isPlay) {
        ...
        Amplitude.init({
            songs: SONGS,
            callbacks: {
                song_change: function () { return handleSongChange(); }
            }
        });
        isPlay && play();
        handleSongChange();
    ...
    var mediaSession = function () {
        var _a, _b;
        var nextTrack = function () { return SONGS.length >= 2
            ? Amplitude.next(Amplitude.getActivePlaylist() || '')
            : enqueueSnackbar('Song lineup is empty'); };
        var prevTrack = function () { return SONGS.length >= 2
        ...
        if ('mediaSession' in navigator) {
            ...
            MEDIA.metadata = new window.MediaMetadata({
                ...
                artist: activeSong.artists
                    ? (_a = activeSong.artists) === null || _a === void 0 ? void 0 : _a.map(function (artist) { return artist.name; }).join(',')
                    : '',
                album: activeSong.album ? (_b = activeSong.album) === null || _b === void 0 ? void 0 : _b.name : '',
                artwork: ARTWORK_SIZES.map(function (size) { return ({
            ...
            if (SONGS.length >= 1 && !MEDIA_CONTROLS.playPause) {
                ...
                MEDIA.setActionHandler('play', function () { return handleMediaClick(true); });
                MEDIA.setActionHandler('pause', function () { return handleMediaClick(); });
            }
    ...
    var onScroll = function () {
        ...
        var externalPage = (scrollY >= document.body.offsetHeight)
    ...
    // Pause active song
    var pause = function () { return Amplitude.pause(); };
    // Play active song
    var play = function () { return Amplitude.play(); };
    var playAll = function (playlist) {
        var songList = playlist.map(function (song) { return getSongObject(song); });
        if (SONGS.length) {
    ...
    var toggleBodyClass = function () {
        var externalPage = (window.innerWidth < 576)
            && !pathname.startsWith('/music')
            && SONGS.length > 0;
        document.body.classList.toggle('player-added', externalPage);
    };
